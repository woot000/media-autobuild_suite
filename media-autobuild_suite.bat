...
set basepackages=cmake dlfcn libpng nasm pcre tools-git yasm ninja pkgconf meson ccache jq gettext-tools
...
if [0]==[%archINI%] (
    ...
    echo. 2 = MINGW64 [recommended]      6 = MINGW32 and MINGW64
    echo. 3 = CLANG32                    7 = CLANG32 and CLANG64
    echo. 4 = CLANG64                    8 = All environments listed
    echo.
    echo -------------------------------------------------------------------------------
    echo -------------------------------------------------------------------------------
...
--xvc=%xvc% --vlc=%vlc% --libavif=%libavif% --jpegxl=%jpegxl% --gimp=%gimp% --av1an=%av1an% ^
--ffmpegPath=%ffmpegPath% --exitearly=%MABS_EXIT_EARLY%
    ...
    if %msysEnv%==CLANG set "MSYSTEM=CLANG64"
