...
timeStamp noMintty noUninstall ccache svthevc svtav1 svtvp9 xvc jo vlc CC jpegxl vvenc vvdec ffmpegPath ^
gimp gimpextra archflags optflags sizeflags
...
if [0]==[%noUninstallINI%] ( set noUninstallF=2 ) else set "noUninstallF=%noUninstallINI%"
if %noUninstallF%=="" set "noUninstall=n"
if %noUninstallF%==1 set "noUninstall=y"
if %noUninstallF%==2 set "noUninstall=n"
if %noUninstallF% GTR 2 set "noUninstall=n"

if [0]==[%archflagsINI%] ( set archflagsF=2 ) else set "archflagsF=%archflagsINI%"
...
if [0]==[%optflagsINI%] ( set optflagsF=2 ) else set "optflagsF=%optflagsINI%"
...
if [0]==[%sizeflagsINI%] ( set sizeflagsF=6 ) else set "sizeflagsF=%sizeflagsINI%"
if %sizeflagsF%=="" set "sizeflags= " && set "sizelinkerflags= "
if %sizeflagsF%==1 set "sizeflags= " && set "sizelinkerflags=-s"
if %sizeflagsF%==2 set "sizeflags=-fdata-sections -ffunction-sections" && set "sizelinkerflags=-Wl,--gc-sections"
if %sizeflagsF%==3 set "sizeflags=-fdata-sections -ffunction-sections" && set "sizelinkerflags=-Wl,--as-needed -Wl,--gc-sections"
if %sizeflagsF%==4 set "sizeflags=-fdata-sections -ffunction-sections" && set "sizelinkerflags=-Wl,--gc-sections -s"
if %sizeflagsF%==5 set "sizeflags=-fdata-sections -ffunction-sections" && set "sizelinkerflags=-Wl,--as-needed -Wl,--gc-sections -s"
if %sizeflagsF%==6 set "sizeflags= " && set "sizelinkerflags= "
