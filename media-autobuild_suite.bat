...
:: basic options
set gimp_options_basic=enable-console-bin enable-multiproc ^
...
zlib gimp ffmpegPath pkgUpdateTime archflags optflags sizeflags
...
if [0]==[%archINI%] (
    ...
    echo. 1 = MINGW32                    3 = UCRT64 [recommended]
    echo. 2 = CLANG64                    4 = All environments listed
    echo.
    echo -------------------------------------------------------------------------------
    echo -------------------------------------------------------------------------------
...
if %selectSystem%==2 set "msysEnv=CLANG64"
if %selectSystem%==3 set "msysEnv=UCRT64"
if %selectSystem%==4 set "msysEnv=ALL"
if %selectSystem% GTR 4 GOTO selectSystem
...
if not exist %instdir%\mintty.lnk (
    ...
    (
        ...
        if %msysEnv%==ALL echo.link.Arguments = "-full-path -ucrt64 -where .."
...
if exist %fstab%. (
    ...
    findstr mingw64 %fstab% >nul 2>&1 && set "removefstab=yes"
...
if %msysEnv%==MINGW32 call :getmingw mingw
if %msysEnv%==CLANG64 call :getmingw clang
if %msysEnv%==UCRT64 call :getmingw ucrt
if %msysEnv%==ALL (
    call :getmingw mingw
    call :getmingw clang
    call :getmingw ucrt
...
if %msysEnv%==MINGW32 call :writeProfile mingw
if %msysEnv%==CLANG64 call :writeProfile clang
if %msysEnv%==UCRT64 call :writeProfile ucrt
if %msysEnv%==ALL (
    call :writeProfile mingw
    call :writeProfile clang
    call :writeProfile ucrt
)
...
--gimp=%gimp% --ffmpegPath=%ffmpegPath% --exitearly=%MABS_EXIT_EARLY%
    ...
    if %msysEnv%==ALL set "MSYSTEM=UCRT64"
...
set destname=
if %1==mingw (
    set destname=local32-mingw
) else if %1==clang (
    set destname=local64-clang
) else if %1==ucrt (
    set destname=local64-ucrt
)
(
    echo.#!/usr/bin/bash
    if %1==mingw (
    ...
    echo.export LOCALDESTDIR='/%destname%'
    echo.
    if %1==mingw (
        echo.bits='32bit'
    ) else (
...
)>%instdir%\%destname%\etc\profile2.local
%instdir%\msys64\usr\bin\dos2unix -q %instdir%\%destname%\etc\profile2.local
goto :EOF
...
if %1==mingw (
    ...
    set envname=MINGW32
    for %%i in (%mingwpackages%) do echo.%%i>>%instdir%\msys64\etc\pac-%1.pk
) else if %1==clang (
    set "compiler=%instdir%\msys64\clang64\bin\clang.exe"
    set prefix=mingw-w64-clang-x86_64-
    set envname=CLANG64
    for %%i in (%clangpackages%) do echo.%%i>>%instdir%\msys64\etc\pac-%1.pk
) else if %1==ucrt (
    set "compiler=%instdir%\msys64\ucrt64\bin\gcc.exe"
    set prefix=mingw-w64-ucrt-x86_64-
    set envname=UCRT64
    for %%i in (%ucrtpackages%) do echo.%%i>>%instdir%\msys64\etc\pac-%1.pk
)
if exist %compiler% set found=1
if %found%==1 GOTO :EOF
echo.-------------------------------------------------------------------------------
echo.install %envname% compiler
echo.-------------------------------------------------------------------------------
(
    echo.printf '\033]0;install %envname% compiler\007'
    echo.[[ "$(uname)" = *6.1* ]] ^&^& nargs="-n 4"
    echo.sed 's/^^/%prefix%/g' /etc/pac-%1.pk ^| xargs $nargs pacman -Sw --noconfirm --ask=20 --needed
    echo.sed 's/^^/%prefix%/g' /etc/pac-%1.pk ^| xargs $nargs pacman -S --noconfirm --ask=20 --needed
    echo.sleep 3
    echo.exit
)>%build%\mingw.sh
call :runBash %envname%.log /build/mingw.sh

if exist %compiler% set found=1
if %found%==0 (
    echo -------------------------------------------------------------------------------
    echo.
    echo.%envname% compiler isn't installed; maybe the download didn't work
