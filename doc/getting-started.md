...
After the suite is done, the binary files will appear in the local[64|32]-[mingw|clang|ucrt] folder folders under bin-audio, bin-global, and bin-video.
...
If you want to rebuild the programs from scratch at any time due to an error or something else, delete the local[64|32]-[mingw|clang|ucrt] folder folder and remove any \*-git and \*-svn folders from the build folder. Finally rerun the batch script, it should redownload all of those folders and rebuild into a new local[64|32]-[mingw|clang|ucrt] folder.
