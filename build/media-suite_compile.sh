...
rust_wanted=false
rust_packages=(ripgrep rav1e dssim libavif dovitool hdr10plustool av1an gimp)
if [[ "$ripgrep|$rav1e|$dssim|$libavif|$dovitool|$hdr10plustool|$av1an|$gimp" = *y* ]] ||
...
if [[ $bits = 64bit ]]; then
    ...
    if [[ $gimp = n && -n $_reenable_gimp ]]; then
        gimp=$_reenable_gimp
        rust_wanted=true
        unset _reenable_gimp
    fi
fi
...
if { [[ $ffmpeg != no ]] && enabled libaribb24; } || [[ $gimp = y ]]; then
    ...
    if [[ $ffmpeg != no ]] && enabled libaribb24 && 
        do_vcs "$SOURCE_REPO_ARIBB24"; then
...
if { gimp_enabled openexr || gegl_enabled openexr; }; then
    ...
    do_cmake_targets_error_to_warning libdeflate
...
if { { [[ $jpegxl = y ]] || { [[ $ffmpeg != no ]] && enabled libjxl; } } ||
    ...
    if do_vcs "$SOURCE_REPO_LIBJXL"; then
        ...
        CXXFLAGS+=" -DJXL_CMS_STATIC_DEFINE -DJXL_STATIC_DEFINE -DJXL_THREADS_STATIC_DEFINE $($PKG_CONFIG --cflags zlib)" \
            LDFLAGS+=" $($PKG_CONFIG --libs zlib)" \
            do_cmakeinstall global -D{BUILD_TESTING,JPEGXL_ENABLE_{BENCHMARK,DOXYGEN,MANPAGES,SKCMS,EXAMPLES,JPEGLI}}=OFF \
...
if [[ $gimp = y ]]; then
    ...
    do_pacman_remove python-rst2pdf gtk3 lua pango libcaca fontconfig libsecret glib2 freetype fontconfig harfbuzz fribidi gtk-doc
    ...
    local _py_ext=x86_64_
    [[ $MSYSTEM =~ CLANG ]] && _py_ext+=ucrt_llvm
    [[ $MSYSTEM =~ UCRT ]] && _py_ext+=ucrt_gnu
    ...
    _check=(bin/lib{charset-1,iconv-2}.dll lib{charset,iconv}.dll.a)
    if files_exist "${_check[@]}"; then
        do_print_status "libiconv-1.19" "$green" "Up-to-date"
    elif do_wget -h 88dd96a8c0464eca144fc791ae60cd31cd8ee78321e67397e25fc095c4a19aa6 \
        "https://ftp.gnu.org/pub/gnu/libiconv/libiconv-1.19.tar.gz"; then
        do_uninstall include/{iconv,libcharset,localcharset}.h "${_check[@]}"
        do_pacman_install gperf
        do_patch "https://raw.githubusercontent.com/msys2/MINGW-packages/refs/heads/master/mingw-w64-libiconv/0002-fix-cr-for-awk-in-configure.all.patch"
        do_patch "https://raw.githubusercontent.com/msys2/MINGW-packages/refs/heads/master/mingw-w64-libiconv/fix-pointer-buf.patch"
        do_patch "https://raw.githubusercontent.com/msys2/MINGW-packages/refs/heads/master/mingw-w64-libiconv/0003-add-cp65001-as-utf8-alias.patch"
        do_make -f Makefile.devel all
        create_build_dir
        config_path=.. do_configure --enable-shared --disable-static \
            --enable-{extra-encoding,relocatable,nls,silent-rules} --disable-rpath
        do_make
        do_makeinstall
        do_checkIfExist
    fi

    _check=(bin/lib{asprintf-0,intl-8}.dll lib{asprintf,intl}.dll.a)
    if files_exist "${_check[@]}"; then
        do_print_status "gettext-1.0" "$green" "Up-to-date"
    elif do_wget -h 71132a3fb71e68245b8f2ac4e9e97137d3e5c02f415636eb508ae607bc01add7 \
        "https://ftp.gnu.org/pub/gnu/gettext/gettext-1.0.tar.xz"; then
        do_uninstall include/{autosprintf,libintl}.h lib/gettext "${_check[@]}"
        do_patch "https://raw.githubusercontent.com/msys2/MINGW-packages/refs/heads/master/mingw-w64-gettext/0005-Fix-compilation-of-pthread_sigmask.c.patch"
        do_patch "https://raw.githubusercontent.com/msys2/MINGW-packages/refs/heads/master/mingw-w64-gettext/122-Use-LF-as-newline-in-envsubst.patch"
        do_patch "https://raw.githubusercontent.com/msys2/MINGW-packages/refs/heads/master/mingw-w64-gettext/0021-replace-fsync.patch"
        do_patch "https://raw.githubusercontent.com/msys2/MINGW-packages/refs/heads/master/mingw-w64-gettext/0022-libasprintf.patch"
        do_patch "https://raw.githubusercontent.com/msys2/MINGW-packages/refs/heads/master/mingw-w64-gettext/0024-disable-gnu-format.patch"
        do_patch "https://raw.githubusercontent.com/msys2/MINGW-packages/refs/heads/master/mingw-w64-gettext/0030-fix-build-with-mingw-w64-clang.patch"
        do_autogen --skip-gnulib
        create_build_dir
        config_path=.. do_configure --enable-shared --disable-static --enable-relocatable \
            --enable-threads=win32 --disable-{csharp,java,native-java,openmp,silent-rules} \
            --with-included-{libcroco,libunistring,libxml,glib} --with-libncurses-prefix=${MINGW_PREFIX} --without-{cvs,emacs,git} \
            lt_cv_deplibs_check_method='pass_all' gl_cv_func_mkdir_trailing_dot_works=yes
        sed -s "s|${MINGW_PREFIX}|$(cygpath -m ${MINGW_PREFIX})|g" -i gettext-tools/config.h
        do_make
        do_makeinstall
        # remove dozens of unnecessary tools
        rm -f ${_gimp_dir}/bin/{msg{attrib,cat,cmp,comm,conv,en,exec,filter,fmt,grep,init,merge,pre,unfmt,uniq},recode-sr-latin,xgettet}.exe
        rm -f ${_gimp_dir}/bin/{autopoint,gettextize,po-fetch,spit} ${_gimp_dir}/bin/lib{gettextpo,gettextsrc-1,gettextlib-1,textstyle}-0.dll
        rm -f ${_gimp_dir}/lib/lib{gettextpo,gettextsrc,gettextlib,textstyle}.{dll.,l}a
        rm -rf ${_gimp_dir}/share/doc/gettext
        do_checkIfExist
    fi
    ...
    if do_vcs "$SOURCE_REPO_GLIB"; then
        ...
        get_custom_flags userflags
        extra_script pre meson
        ( [[ -n $userflags ]] && export "${userflags[@]}"; CC=${CC/ccache /}.bat CXX=${CXX/ccache /}.bat \
            log "meson" meson setup .. --default-library=shared --buildtype=release \
    ...
    local _vala_ver=0.56.19
    local _vala_hash=5ad7cbbfcc0de61b403d6797c9ef60455bfbebd8e162aec33b5b0b097adfb9d5
    ...
    if do_vcs "$SOURCE_REPO_SHARED_MIME_INFO"; then
        ...
        CXXFLAGS+=" -DLIBXML_STATIC" LDFLAGS+=" $(xml2-config --libs) -llzma -lz -lws2_32" \
            do_mesoninstall -D{build-spec,build-tests,update-mimedb}=false
    ...
    if do_vcs "$SOURCE_REPO_HARFBUZZ"; then
        ...
        do_pacman_install python-setuptools ragel
    ...
    _check=(pygobject-3.0.pc pygobject-3.0/pygobject.h lib/python${cpython_major_ver}/site-packages/gi/__init__.py)
    if do_vcs "$SOURCE_REPO_PYGOBJECT"; then
        do_uninstall "${_check[@]}" lib/python${cpython_major_ver}/site-packages/{gi,PyGObject*info}
        do_mesoninstallshared -Dtests=false
        log "python compileall" ${MINGW_PREFIX}/bin/python -m compileall \
    ...
    if do_vcs "$SOURCE_REPO_LIBMYPAINT"; then
        ...
        do_pacman_install perl-xml-parser
    ...
    if do_vcs "$SOURCE_REPO_PANGO"; then
        ...
        do_mesoninstallshared -D{cairo,fontconfig,freetype}=enabled \
            -Dintrospection=enabled -Dbuild-{examples,testsuite}=false
        do_checkIfExist
    fi
    ...
    if gimp_enabled wmf && do_vcs "$SOURCE_REPO_LIBWMF"; then
        ...
        sed -e "s;LIBWMF_EXPORT __declspec(dllexport);LIBWMF_EXPORT;" \
            -e "s;LIBWMF_EXPORT __declspec(dllimport);LIBWMF_EXPORT;" \
            -i src/player/color.h -i include/libwmf/color.h
        do_autoreconf
        CFLAGS+=" -I$(cygpath -pm ${_gimp_dir}/../include) -Wno-error=implicit-function-declaration" \
    ...
    if do_vcs "$SOURCE_REPO_LIBRSVG"; then
        ...
        local _extra_ldflags=("-Wl,--allow-multiple-definition") # multiple definition of `GetLastError'
    ...
    if do_vcs "$SOURCE_REPO_LIBPROXY"; then
        ...
        get_custom_flags userflags
        extra_script pre meson
        ( [[ -n $userflags ]] && export "${userflags[@]}"; CC=${CC/ccache /}.bat \
            log "meson" meson setup .. --default-library=shared --buildtype=release \
    ...
    if do_vcs "$SOURCE_REPO_GLIB_NETWORKING"; then
        ...
        CFLAGS+=" -DGNUTLS_INTERNAL_BUILD" do_mesoninstallshared -Dopenssl=enabled -Dtests=false
    ...
    if do_vcs "$SOURCE_REPO_GEXIV2"; then
        ...
        LDFLAGS+=" $($PKG_CONFIG --libs --static exiv2)" \
    ...
    if gimp_enabled ghostscript; then
        ...
        local _ghostscript_ver=10.07.1
        local _ghostscript_hash=1cdb766de8db8f1e589c817f09c5855ea5f65dfc8540e465a69ac14c18416025
    ...
    if do_vcs "$SOURCE_REPO_DRMINGW"; then
        ...
        # allow pthreads
        sed -i 's;message (SEND_ERROR "Win32;message (WARNING "Win32;' CMakeLists.txt
        do_cmakeinstallgimpdir
        rm -rf $_gimp_dir/{doc,sample}/
        do_checkIfExist
    fi

    local _adwaita_ver=50.0
    local _adwaita_hash=fac6e0401fca714780561a081b8f7e27c3bc1db34ebda4da175081f26b24d460
    ...
    _check=(bin/gimp{,-debug-tool,-test-clipboard,tool}-3.3.exe bin/gimp-script-fu-interpreter-3.0.exe
    ...
    opt_exists GIMP_OPTS "-Denable-console-bin=true" && _check+=(bin/gimp-console-3.3.exe)
    if do_vcs "$SOURCE_REPO_GIMP"; then
        ...
        if gimp_enabled fits; then
            ...
            _extra_ldflags+=("$($PKG_CONFIG --libs --static libbrotlidec libidn2 libnghttp2 libnghttp3 libssh2 openssl)")
...
run_builds() {
    ...
    if [[ $msysEnv == ALL || $msysEnv == MINGW32 ]]; then
