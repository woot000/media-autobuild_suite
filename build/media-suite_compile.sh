...
if [[ $gimp = y ]]; then
    ...
    local _ver=2.0
    [[ $bits = 32bit ]] && _ver=1.0 && SOURCE_REPO_MYPAINT_BRUSHES="${SOURCE_REPO_MYPAINT_BRUSHES%%#*}"
    _check=(mypaint-brushes-$_ver.pc)
    if do_vcs "$SOURCE_REPO_MYPAINT_BRUSHES"; then
        do_uninstall share/mypaint-data "${_check[@]}"
        do_autogen
        do_separate_confmakeinstall
        cp ${_gimp_dir}/{share,lib}/pkgconfig/mypaint-brushes-$_ver.pc
        do_checkIfExist
    fi
    unset _ver
    ...
    if do_vcs "$SOURCE_REPO_BABL"; then
        ...
        do_patch "https://raw.githubusercontent.com/msys2/MINGW-packages/refs/heads/master/mingw-w64-babl/001-file-open-unicode.patch"
    ...
    if gimp_enabled ghostscript; then
        ...
        local _ghostscript_ver=10.05.1
        local _ghostscript_hash=22f2bdca15c28830c9715cddc5c296ea66898bfdab0b604a4e0bcfeb03af6cad
    ...
    local _adwaita_ver=48.0
    local _adwaita_hash=847068888650d9673115be6dbf2bfdc31a46aebc528a6a9db4420e60e656b8d4
    ...
    local _iso_codes_ver=4.18.0
    local _iso_codes_hash=511f67bf4b51aa77f17c45adbff533242b50f1e370fe49a5706b6341902fac87
    ...
    _check=(bin/gimp{,-debug-tool,-test-clipboard,tool}-3.1.exe bin/gimp-script-fu-interpreter-3.0.exe
    ...
    opt_exists GIMP_OPTS "-Denable-console-bin=true" && _check+=(bin/gimp-console-3.1.exe)
