...
if [[ "$ripgrep|$rav1e|$dssim|$libavif|$dovitool|$hdr10plustool|$gimp" = *y* ]] || enabled librav1e; then
...
if gegl_enabled libraw; then
    ...
    [[ $CC == *gcc* ]] && do_pacman_remove openmp
...
if { [[ $openexr = y ]] || { gimp_enabled openexr || gegl_enabled openexr; }; }; then
    ...
    if do_vcs "$SOURCE_REPO_OPENEXR"; then
        ...
        [[ $openexr = y || $standalone = y ]] && extracommands+=(-DOPENEXR_BUILD_TOOLS=ON)
...
{ enabled librav1e || [[ $libavif = y || $gimp = y ]]; } && do_pacman_install cargo-c
...
if { [[ ! $x265 = n ]] || { [[ $libheif = y ]] || gimp_enabled heif; }; } &&
    ...
    if [[ $x265 =~ (o12|s|d|y) ]] || { [[ $libheif = y ]] || gimp_enabled heif; }; then
        ...
        elif [[ $x265 = d || $x265 = y ]] || { [[ $libheif = y ]] || gimp_enabled heif; }; then
            do_x265_cmake "12-bit lib for multilib" $assembly -DEXPORT_C_API=OFF -DMAIN12=ON
            cp libx265.a ../8bit/libx265_main12.a
        fi
    fi

    if [[ $x265 =~ (o10|s|d|y) ]] || { [[ $libheif = y ]] || gimp_enabled heif; }; then
    ...
    if [[ $x265 =~ (o8|s|d|y) ]] || { [[ $libheif = y ]] || gimp_enabled heif; }; then
...
build_libheif() {
    local cmd=(do_cmakeinstall global)
    local dir=""
    local heifexec=n
    ...
    _deps=(${dir}lib{aom,dav1d,x265}.a)
    { [[ $libheif = y ]] && ! gimp_disabled jpeg2000; } && _deps+=(${dir}libopenjp2.a)
    [[ $other265 = y ]] && _deps+=(${dir}libkvazaar.a)
    [[ $bits = 64bit && $svtav1 = y ]] && _deps+=(${dir}libSvtAv1Enc.a)
    ...
    unset cmd dir heifexec
}

[[ $libheif = y ]] && build_libheif
...
if [[ $gimp = y ]]; then
    ...
    do_pacman_remove gtk3 lua pango libcaca fontconfig libsecret glib2 freetype fontconfig harfbuzz fribidi
    ...
    if do_vcs "$SOURCE_REPO_GLIB"; then
        ...
        CC=${CC/ccache /}.bat CXX=${CXX/ccache /}.bat \
            log "meson" meson setup .. --default-library=shared --buildtype=release \
            --prefix="${_gimp_dir}" --backend=ninja -Dtests=false -Dintrospection=enabled -Dlibelf=disabled
        extra_script post meson
        do_ninja
    ...
    if do_vcs "$SOURCE_REPO_SHARED_MIME_INFO"; then
        ...
        sed -i '/	LIBXML_TEST_VERSION;/d' src/update-mime-database.cpp
        CXXFLAGS+=" -DLIBXML_STATIC" LDFLAGS+=" $(xml2-config --libs) -llzma -lz" \
    ...
    if do_vcs "$SOURCE_REPO_LIBRSVG"; then
        ...
        # librsvg MUST be built shared, otherwise the adwaita icon PNGs will not be generated later on
        export WINAPI_NO_BUNDLED_LIBRARIES=1
        create_build_dir
        do_mesonshared -D{introspection,vala}=enabled -Ddocs=disabled -Dtests=false -Dtriplet=${MSYSTEM_CARCH}-pc-windows-gnu
        do_ninja
        mv -f gdk-pixbuf-loader/{,lib}pixbufloader_svg.dll
        do_ninjainstall
        do_checkIfExist
    fi
    ...
    if gegl_enabled graphviz && _deps=(lib{cairo,glib-2.0,pango-1.0,rsvg-2}.dll.a) &&
        ...
        do_autogen
        do_separate_confmakeinstall --enable-static --disable-shared \
    ...
    if do_vcs "$SOURCE_REPO_POPPLER"; then
        ...
        # workaround to deal with poppler not finding tiff the first time cmake runs
    ...
    grep_or_sed Libs.private "$_gimp_dir/lib/pkgconfig/poppler.pc" '/Libs:/aLibs.private: -lstdc++'
    ...
    if gegl_enabled libav; then
        ...
        elif do_vcs "$ffmpegPath#branch=release/6.1"; then
            # specifying release 6.1 avoids this gegl compile error: ../operations/external/ff-load.c:556:64: error: 'AVCodecParameters' has no member named 'channels'
    ...
    if gimp_enabled ghostscript; then
        ...
        if files_exist "${_check[@]}"; then
            ...
            sed -i 's;"openjpeg.h".*;&\n#include "opj_config.h";' base/sjpx_openjpeg.h
    ...
    _check=(bin/gimp{,-debug-tool,-test-clipboard,tool}-2.99.exe bin/gimp-script-fu-interpreter-3.0.exe
    ...
    opt_exists GIMP_OPTS "-Denable-console-bin=true" && _check+=(bin/gimp-console-2.99.exe)
    if do_vcs "$SOURCE_REPO_GIMP"; then
        ...
        [[ $bits = 32bit ]] && _extra_ldflags+=("-Wl,--large-address-aware")
        gimp_disabled lua && sed -i 's;lua.found();have_lua;' extensions/meson.build
        ...
        CFLAGS+=" -Wno-incompatible-pointer-types -DOPJ_STATIC" LDFLAGS+=" ${_extra_ldflags[@]}" \
