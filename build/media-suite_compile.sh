...
if [[ $gimp = y ]]; then
    ...
    if do_vcs "$SOURCE_REPO_EXIV2"; then
        ...
        if [[ $MSYSTEM =~ MINGW ]]; then
            {
                git revert --no-edit a513534e || git revert --abort # _dupenv_s does not exist on gcc
            } > /dev/null 2>&1
            sed -i '/if(MINGW)/,+6d' cmake/mainSetup.cmake # heinous, but it works
        fi
    ...
    if gimp_enabled ghostscript; then
        ...
        local _ghostscript_ver=10.07.0
        local _ghostscript_hash=ddace4e1721f967a55039baff564840225e0baa1d4f5432247ca1ccd1473b7c1
