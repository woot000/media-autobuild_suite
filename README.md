...
- NTFS drive
- 8GB+ for MINGW64, 32GB+ disk space for all environments
- 4GB+ RAM
...
- Wait a little bit, and hopefully after a while you'll find all your "*.exe" tools under local(64|32)-(mingw|clang|ucrt)-\bin-(audio|global|video)
...
- If you select the option to choose your own FFmpeg/mpv optional libraries, this file will contain options that get sent to FFmpeg/mpv's configure script before compiling. Edit these files as you wish to get a smaller FFmpeg/mpv without features you don't need or with additional features not compiled by default, if supported.

`/build/gegl_options.txt` & `/build/gimp_options.txt`

- If you select the option to build GIMP, these two files will contain options that get sent to gegl/GIMP's Meson build system before compiling. Edit these files as you wish to get a smaller gegl/GIMP without features you don't need. The syntax of these two files differs from `ffmpeg_options.txt` and `mpv_options.txt`, using Meson syntax as opposed to configure script syntax. (E.g. `-Doption=true/-Dfeature=enabled` instead of `--enable-option|feature`)
...
- To use custom environment variables, create a text file with the filename: `<repository's name>_flags.txt`.
  ...
  - Examples: `ffmpeg_flags.txt` will be used for all environments, `SVT_AV1_flags_CLANG64.txt` takes priority over `SVT_AV1_flags.txt` if building with CLANG64, but otherwise the latter will be used for all other environments.
